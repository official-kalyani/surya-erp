...
$lineData = array("Slno","Asset No.","Serial No.","Name Of Asset","Category","Sub Category","Date Of Purchase","Supplier","Invoice No.","QTY","Item Sub Type"," Current Location","Used By", "From Date", "Status");
...
if(isset($_GET['sdate']) || isset($_GET['tdate']) || isset($_GET['spname']) )
    {
               ...
               $search = $_GET['select_search'];
                ...
                if($search == 1)
                {
                   ...
                   $abc = mysqli_query($con,"SELECT x.*, x.id as xid, y.*, y.id as id FROM aset_stock_entry  x, vw_aset_stock_assaignment y WHERE x.auto_serial = y.asset_no AND x.asset_type = 'User' AND y.project_site LIKE '%$spname%' AND y.status = '1' GROUP BY x.auto_serial");
                    ...
                    {
                      $result = mysqli_query($con,"SELECT x.*, x.id as xid, y.*, y.id as id FROM aset_stock_entry  x, vw_aset_stock_assaignment y WHERE x.auto_serial = y.asset_no AND x.asset_type = 'User' AND y.project_site LIKE '%$spname%' AND y.status = '1' GROUP BY x.auto_serial");

                    }
                    else{
                          $result = mysqli_query($con,"SELECT x.*, x.id as xid, y.*, y.id as id FROM aset_stock_entry  x, vw_aset_stock_assaignment y WHERE x.auto_serial = y.asset_no AND x.asset_type = 'User' AND x.godowo LIKE '%$spname%' m GROUP BY x.auto_serial");
               ...
               else if($search == 14)
              {
                ...
                $result = mysqli_query($con,"SELECT * FROM `aset_stock_entry` WHERE  status = '1' AND asset_type = 'User' AND description LIKE '%$spname%'");
                    ...
                    {
                $result = mysqli_query($con,"SELECT * FROM `aset_stock_entry` WHERE  status = '1' AND asset_type = 'User'"); 
                    } 
