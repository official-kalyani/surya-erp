                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading" style="height:40px;">
                               <form name="text_search" method="GET">
                                 <span><button type="button" onClick='javascript:location.href="stock_view.php"' class="btn btn-default btn-md active btn-xs" style="font-weight: bold;">User Asset List</button>
                              <a href="stock_entry.php" class="btn btn-success btn-xs" style="text-decoration:none;" ><b>ADD  STOCK ENTRY </b></a>
                                    ...
                                    if (in_array($_SESSION['ERP_SESS_ID'], $assetID)){
                                      ?>  
                                  <button type="button" onClick='javascript:location.href="admin_asset_list.php"' class="btn btn-primary btn-xs" style="text-decoration:none;">ADMIN ASSET LIST</button>
                                  <?php
                                 }
                                 ?>
                                </span>
    ...
    {   
       ...
       $pagination.= "</div>\n";       
               ...
               if(isset($_POST['submitdt']))
                ...
                if($search == 1)
                {
                    ...
                    $result = mysqli_query($con,"SELECT * FROM aset_stock_entry WHERE status = '1' AND `asset_type` = 'User' AND inv_no LIKE '%$spname%' ");
                     ...
                     {
                     $spname = mysqli_real_escape_string($con,$_GET['spname']);
                    $result = mysqli_query($con,"SELECT * FROM aset_stock_entry WHERE status = '1' AND `asset_type` = 'User' AND qty LIKE '%$spname%' ");
                }
                else if($search == 8)
                     {
                     $spname = mysqli_real_escape_string($con,$_GET['spname']);
               ...
               else if($search == 14)
              {
                ...
                $result = mysqli_query($con,"SELECT * FROM aset_stock_entry WHERE status = '1' AND description LIKE '%$spname%' AND `asset_type` = 'User'  "); 
