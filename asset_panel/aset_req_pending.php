    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h4 class="page-header">Maintenance Pending Request</h4>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">
                              <!--   <div class="row"> -->
                                  ...
                                  <button onClick='javascript:location.href="aset_req_pending.php"' class="btn btn-default btn-md active" style="font-weight: bold;">PENDING</button>
                                ...
                                if(isset($_POST['submitnm']))    { // Show Only if Search Action Performed ?>
                                  <!--   <span style="float:left; padding-right:10px;"> -->
                                    ...
                                    <button onClick='javascript:location.href="aset_req_pending.php"' class="btn btn-primary btn-xs">Refresh</button>
               ...
               if(isset($_POST['submitdt']))
                    {
                         ...
                         $pdate = $_POST['tdate'];
                         $SESSION['pdate']= $pdate;
                         $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.status as xstatus,x.date as xdate ,y.*,z.fullname  FROM aset_req_details x, aset_req_to y , mstr_emp z WHERE x.id = y.req_id AND y.status = '0' AND x.status = '0' AND y.req_to = '$emp_id' AND x.req_by = z.id GROUP BY x.asset_req_no ORDER BY x.id limit $start,$limit"); 
                    }
                ...
                if($search == '1')
                    ...
                    else if($search == '3')
                    {
                     ...
                     else if($search == '4')
                     {
                      ...
                      if($search != 4)
                      {
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z WHERE x.req_by = y.id AND z.req_to = '$emp_id' AND z.req_id = x.id AND x.status = '0' AND z.status = '0'AND $qut GROUP BY x.asset_req_no ORDER BY x.id"); 
                      }
                      else{
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z WHERE z.req_to = y.id  AND z.req_id = x.id  AND x.status = '0' AND $qut  AND z.status = '0' GROUP BY x.asset_req_no ORDER BY x.id "); 
                      }
