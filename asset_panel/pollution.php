...
<html lang="en">
    <head>
        ...
        <title>Pollution Update : Suryam Group</title>
    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h4 class="page-header">Pollution Field Update</h4>
                                ...
                                <div>
                                    <form name='search' method='get' class="row-flex">
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            ?>
                                                <a href="pollution.php" class="btn btn-primary">Refresh</a>
                                            <?php 
                                            } 
                                            ?>
                                        </div>
                                        ...
                                        <div class="col-md-2" style="float: right;">
                                            <a href="download_pollution.php<?php echo $concate.$op;?>pdf=pdf_download" class="btn btn-info">PDF</a>
                                            <a href="download_pollution.php<?php echo $concate.$op;?>excel=excel_download" class="btn btn-primary">EXCEL</a>
                                        </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped">
                                        ...
                                        <?php
                                            ...
                                            if ($emp=='55') 
                                            {
                                                if(isset($_GET['spname']) && isset($_GET['erp_fullname']))
                                                {
                                                    $spname = mysqli_real_escape_string($con,$_GET['spname']); // Prevent SQL-Injection
                                                    ...
                                                    if ($spname!=='' && $erp_fullname!=='' && $ins_status!=='') 
                                                    {
                                                        ...
                                                        $result = mysqli_query($con,"SELECT x.*,y.*,y.id as yid,z.* FROM aset_stock_assaignment x,aset_stock_entry y,aset_stock_entry_vechile z  WHERe poc_valid < cast(now() as date) AND x.asset_no = y.auto_serial AND y.id = z.name_id AND y.auto_serial = '$spname' AND  x.assaign_to = '$erp_fullname' AND x.status = '1'");
                                                        }else{
                                                            $result = mysqli_query($con,"SELECT x.*,y.*,y.id as yid,z.* FROM aset_stock_assaignment x,aset_stock_entry y,aset_stock_entry_vechile z  WHERe poc_valid > cast(now() as date) AND x.asset_no = y.auto_serial AND y.id = z.name_id AND y.auto_serial = '$spname' AND  x.assaign_to = '$erp_fullname' AND x.status = '1'");
                                                        }
                                                    }
                                                    ...
                                                    {
                                                        if ($ins_status == 'inactive') {
                                                            $result = mysqli_query($con,"SELECT x.*,y.*,y.id as yid,z.* FROM aset_stock_assaignment x,aset_stock_entry y,aset_stock_entry_vechile z  WHERe poc_valid < cast(now() as date) AND x.asset_no = y.auto_serial AND y.id = z.name_id AND x.status = '1'");
                                                        }else{
                                                            $result = mysqli_query($con,"SELECT x.*,y.*,y.id as yid,z.* FROM aset_stock_assaignment x,aset_stock_entry y,aset_stock_entry_vechile z  WHERe poc_valid > cast(now() as date) AND x.asset_no = y.auto_serial AND y.id = z.name_id AND x.status = '1'");
                                                        }
                                                    ...
                                                    {
                                                        if ($ins_status == 'inactive') {
                                                            $result = mysqli_query($con,"SELECT x.*,y.*,y.id as yid,z.* FROM aset_stock_assaignment x,aset_stock_entry y,aset_stock_entry_vechile z  WHERe poc_valid < cast(now() as date) AND x.asset_no = y.auto_serial AND y.id = z.name_id AND x.assaign_to = '$erp_fullname' AND x.status = '1'");
                                                        }else {
                                                            $result = mysqli_query($con,"SELECT x.*,y.*,y.id as yid,z.* FROM aset_stock_assaignment x,aset_stock_entry y,aset_stock_entry_vechile z  WHERe poc_valid > cast(now() as date) AND x.asset_no = y.auto_serial AND y.id = z.name_id AND x.assaign_to = '$erp_fullname' AND x.status = '1'");
                                                        }
                                                    }
                                                    ...
                                                    {
                                                        if ($ins_status == 'inactive') {
                                                            $result = mysqli_query($con,"SELECT x.*,y.*,y.id as yid,z.* FROM aset_stock_assaignment x,aset_stock_entry y,aset_stock_entry_vechile z  WHERe  poc_valid < cast(now() as date) AND x.asset_no = y.auto_serial AND y.id = z.name_id AND y.auto_serial = '$spname' AND x.status = '1'");
                                                        }else{
                                                            $result = mysqli_query($con,"SELECT x.*,y.*,y.id as yid,z.* FROM aset_stock_assaignment x,aset_stock_entry y,aset_stock_entry_vechile z  WHERe  poc_valid > cast(now() as date) AND x.asset_no = y.auto_serial AND y.id = z.name_id AND y.auto_serial = '$spname' AND x.status = '1'"); 
                                                        }
                                                    }
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php
                                                            ...
                                                            $queryan = mysqli_query($con,"SELECT x.*,y.fullname  from aset_stock_assaignment x,mstr_emp y WHERE x.assaign_to = y.id AND x.asset_no = '$assaign' ORDER BY x.id DESC LIMIT 1");
                                                    ...
                                                    <td><?php echo $fetch->poc_valid; ?></td>
                                                    <td>
                                                        <?php
                                                            $date = $fetch->poc_valid;
                                                            $today = date('Y-m-d');
                                                            if ($today > $date) 
                                                            {
                                                                echo "Inactive"; 
                                                    ...
                                                    <td><a href="poc_view.php?id=<?php echo $fetch->yid;?>" style="text-decoration:none;" class="btn btn-primary btn-xs" ><b>View</b></a></td>
                                                    <?php
                                                    if ($emp==55) 
                                                    {
                                                        ?>
                                                        <td><a href="poc_edit.php?id=<?php echo $fetch->yid;?>" style="text-decoration:none;" class="btn btn-success btn-xs" ><b>Edit</b></a></td>
                                                        <?php
                                                    }
                                                    ?>
                                                </tr>
