        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                ...
                <!-- Body Starts Here --> 
                                    ...
                                    <div class="col-lg-3">  
                                      ...
                                      <div class="col-lg-3">  
                                        <div class="form-group"> 
                                           ...
                                           <div class="col-lg-3">  
                                                <div class="form-group"> 
                                                    <input type='submit' name="submitdt" value='Search' class="btn btn-success btn-xs">
                                                </div>
                                            </div>
                                            <?php
                    ...
                    {
                        ?>
                         <a href="print.php?sdate=<?php echo $_POST['sdate'];?>&tdate=<?php echo $_POST['tdate'];?>&spname=<?php echo $_POST['spname'];?>&select_search=<?php echo $_POST['select_search'];?>" class="btn btn-warning btn-xs" style="text-decoration:none;" target="_blank"><b>Print</b></a>
                         <a href="download.php?sdate=<?php echo $_POST['sdate'];?>&tdate=<?php echo $_POST['tdate'];?>&spname=<?php echo $_POST['spname'];?>&select_search=<?php echo $_POST['select_search'];?>" class="btn btn-warning btn-xs" style="text-decoration:none;" ><b>Download</b></a>
                         <?php
                    }
                    ...
                    {
                        ?>
                <a href="print.php" class="btn btn-warning btn-xs" style="text-decoration:none;"  target="_blank" ><b>Print</b></a>
                <a href="download.php" class="btn btn-warning btn-xs" style="text-decoration:none;"><b>Download</b></a>
                <?php
                    } ?>
                              ...
                              <!--   <div class="row"> -->
                                ...
                                if(isset($_POST['submitnm']))    { // Show Only if Search Action Performed ?>
                                  <!--   <span style="float:left; padding-right:10px;"> -->
                                    ...
                                    <button onClick='javascript:location.href="stock_report.php"' class="btn btn-primary btn-xs">Refresh</button>
                                ...
                                <?php } ?>
                                  ...
                                  <!--   <div class="col-lg-4" style="float: right;"> -->
                                        <div class="col-lg-2" >
                                        <input type='submit' name="submitnm" value='Search' class="btn btn-success btn-xs" >
                                      <!--   </div> -->
...
// Pagination
    ...
    $query = mysqli_query($con, "SELECT COUNT(*) as `num` from `aset_stock_entry` where `status`='1'");
               ...
               if(isset($_POST['submitdt']))
                    {
                         ...
                         $pdate = $_POST['tdate'];
                         $SESSION['pdate']= $pdate;
                         $result = mysqli_query($con,"SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.date_purchase BETWEEN '$spdate' AND '$pdate' "); 
                    }
                ...
                if($search == 1)
                {
                    ...
                    $result = mysqli_query($con,"SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.auto_serial LIKE '%$spname%'");
                }
                else if($search == 2)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.serial_no LIKE '%$spname%'");
                }
                else if($search == 3)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.description LIKE '%$spname%' ");
                }
                else if($search == 4)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.date_purchase LIKE '%$spname%'");
                }
                 else if($search == 5)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.supplier LIKE '%$spname%'");
                }
                 else if($search == 6)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.inv_no LIKE '%$spname%' ");
                }
                else if($search == 7)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.qty LIKE '%$spname%' ");
                }
                else if($search == 8)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*, x.id as xid, y.*, y.id as id FROM aset_stock_entry  x, vw_aset_stock_assaignment y WHERE x.auto_serial = y.asset_no AND y.project_site LIKE '%$spname%' GROUP BY x.auto_serial");
                }
                else if($search == 9)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*, y.* FROM aset_stock_entry  x, vw_aset_stock_assaignment y WHERE x.auto_serial = y.asset_no AND y.fullname LIKE '%$spname%' GROUP BY x.auto_serial ");
                }
                else if($search == 10)
                     {
                     $spname = mysqli_real_escape_string($con,$_POST['spname']);
                    $result = mysqli_query($con,"SELECT x.*, y.* FROM aset_stock_entry  x, vw_aset_stock_assaignment y WHERE x.auto_serial = y.asset_no AND y.assaign_dt LIKE '%$spname%' GROUP BY x.auto_serial");
                }
                ...
                //      $result = mysqli_query($con,"SELECT * FROM `aset_stock_entry` WHERE  status = '1' AND auto_serial LIKE '%$spname%'");
                    ...
                    {
                $result = mysqli_query($con," SELECT x.*,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id ORDER BY x.auto_serial limit $start,$limit"); 
