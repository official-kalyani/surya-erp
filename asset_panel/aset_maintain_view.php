...
<?php  $assetID = array("2","74","316","215","1","55","330","572"); ?>
    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h4 class="page-header">Maintenance Request</h4>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                                  ...
                                  <button onClick='javascript:location.href="aset_req_pending.php"' class="btn btn-warning btn-xs" style="font-weight: bold;">PENDING</button>
                                   ...
                                   <?php
                                    if (in_array($_SESSION['ERP_SESS_ID'], $assetID)){
                                      ?>
                                   <button onClick='javascript:location.href="aset_maintain_admin.php"' class="btn btn-info btn-xs" style="font-weight: bold;">Admin Asset List</button>
                                   <?php
                                 }
                                 ?>
...
// Pagination
    ...
    $query = mysqli_query($con, "SELECT COUNT(*) as `num`,x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z,aset_stock_entry w WHERE x.req_by = y.id AND z.req_id = x.id  AND x.asset_id = w.auto_serial AND w.asset_type = 'User' ");
  }

  else{
    $emp_id = $_SESSION['ERP_SESS_ID'];
 $query = mysqli_query($con, "SELECT COUNT(*) as `num`, x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z WHERE x.req_by = y.id AND z.req_id = x.id ");
  }  
                ...
                // Prevent SQL-Injection
                    ...
                    else if($search == '3')
                    {
                     ...
                     else if($search == '4')
                     {
                      ...
                      if($search != 4)
                      {
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z,aset_stock_entry w WHERE x.req_by = y.id AND z.req_id = x.id AND $qut AND x.asset_id = w.auto_serial AND w.asset_type = 'User' GROUP BY x.asset_req_no ORDER BY x.id"); 
                      }
                      else{
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z,aset_stock_entry w WHERE z.req_to = y.id  AND z.req_id = x.id AND $qut  AND z.status = '0' AND x.asset_id = w.auto_serial AND w.asset_type = 'User' GROUP BY x.asset_req_no ORDER BY x.id "); 
                      }
                    }
                    
                    else // For Normal
                    {
                $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z,aset_stock_entry w WHERE x.req_by = y.id AND z.req_id = x.id AND x.asset_id = w.auto_serial AND w.asset_type = 'User' GROUP BY x.asset_req_no ORDER BY x.date limit $start,$limit"); 
...
?>
                        ...
                        $id_req = $fetch->xid;
                        // echo $id_req;
                        $query = mysqli_query($con,"SELECT x.*,y.fullname FROM aset_req_to x, mstr_emp y WHERE x.req_id = '$id_req' AND x.req_to = y.id AND y.fullname != 'Others' ORDER BY x.id DESC LIMIT 1");
...
<?php $i++; }
  ...
  $emp_id = $_SESSION['ERP_SESS_ID'];
  $query = mysqli_query($con, "SELECT x.*,x.id as xid,y.* FROM aset_req_details  x,aset_req_to y WHERE x.id = y.req_id AND( x.req_by = '$emp_id'  OR y.req_to = '$emp_id') GROUP BY x.`asset_req_no`");
                ...
                if(isset($_POST['submitdt']))
                    {
                         ...
                         $pdate = $_POST['tdate'];
                         $SESSION['pdate']= $pdate;
                         $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z WHERE x.req_by = y.id AND (z.req_to = '$emp_id' OR x.req_by = '$emp_id') AND z.req_id = x.id AND x.date BETWEEN '$spdate' AND '$pdate'  GROUP BY x.asset_req_no ORDER BY x.id"); 
                    }
                    else if(isset($_POST['submitnm'])) // For Search
                    {
                ...
                // Prevent SQL-Injection
                    ...
                    else if($search == '3')
                    {
                     ...
                     else if($search == '4')
                     {
                      ...
                      if($search != 4)
                      {
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z WHERE x.req_by = y.id AND (z.req_to = '330' OR x.req_by = '330') AND z.req_id = x.id AND $qut  GROUP BY x.asset_req_no ORDER BY x.id"); 
                      }
                      else{
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z WHERE z.req_to = y.id  AND z.req_id = x.id AND $qut  AND z.status = '0' GROUP BY x.asset_req_no ORDER BY x.id "); 
                      }
                    }
                    else // For Normal
                    {
                $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z WHERE x.req_by = y.id AND (z.req_to = '$emp_id' OR x.req_by = '$emp_id') AND z.req_id = x.id  GROUP BY x.asset_req_no ORDER BY x.id limit $start,$limit"); 
