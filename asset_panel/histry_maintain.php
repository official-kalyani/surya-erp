          ...
          ?>
            <tbody>
             ...
             if( $ftch_aset_no->sub_type == '6')
             {
                ...
                $result = mysqli_query($con,"SELECT x.*,x.status as xstatus,y.* FROM aset_req_details x,aset_maintainance_req y  WHERE y.asset_id  = x.id AND x.asset_id = '$asset' ORDER BY x.id limit $start,$limit"); 
                // }
            ...
            $ftch_name = mysqli_fetch_object($abs);
                ...
                ?>
                <td><?php if(mysqli_num_rows($abs)>0){ echo $ftch_name->fullname; }else{ echo "Admin";}?></td>
                <td><?php if ($fetch->xstatus == 1) 
                   {
                   echo "approved";
                   }
