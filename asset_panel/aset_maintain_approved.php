...
function getPic()
{
   ...
   if(select_search == '1' || select_search == '6')
   {
    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h4 class="page-header">Maintenance Approved Request</h4>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">
                              <!--   <div class="row"> -->
                                  ...
                                  <button onClick='javascript:location.href="aset_maintain_approved.php"' class="btn btn-default btn-md active" style="font-weight: bold;">Approved</button>
                                </span>
                                ...
                                if(isset($_POST['submitnm']))    { // Show Only if Search Action Performed ?>
                                  <!--   <span style="float:left; padding-right:10px;"> -->
                                    ...
                                    <button onClick='javascript:location.href="aset_maintain_approved.php"' class="btn btn-primary btn-xs">Refresh</button>
                                              ...
                                              <option value>--select by search--</option>
                                                <option value="1">Date</option>
                                                 ...
                                                 <option value="4">Position</option>
                                                  ...
                                                  <option value="6"> Updated Date</option>
                                                   <option value="7"> Updated Position</option>
                                                    <option value="8"> Updated Status</option>
                                             </select>
                                        </div>
                                               <div class="col-lg-4" >
                                               <div id="ldt">
...
// Pagination
 $emp_id = $_SESSION['ERP_SESS_ID'];
    ...
    $query = mysqli_query($con, "SELECT COUNT(*) as `num`FROM aset_req_details x, aset_req_to y , mstr_emp z WHERE x.id = y.req_id AND y.status = '0' AND x.status = '0'  AND x.req_by = z.id ");
       ...
       $pagination.= "</div>\n";       
            ...
            $emp_id = $_SESSION['ERP_SESS_ID'];
               ...
               if(isset($_POST['submitdt']))
                    {
                        $spdate = $_POST['sdate'];
                         ...
                         $pdate = $_POST['tdate'];
                         $SESSION['pdate']= $pdate;
                         $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.status as xstatus,x.date as xdate ,y.*,z.fullname  FROM aset_req_details x, aset_req_to y , mstr_emp z WHERE x.id = y.req_id  AND x.status = '1' AND x.req_by = '$emp_id'  AND x.req_by = z.id AND x.date BETWEEN '$spdate' AND '$pdate' GROUP BY x.asset_req_no ORDER BY x.id limit $start,$limit"); 
                    }
                ...
                if($search == '1')
                    ...
                    else if($search == '3')
                    {
                     ...
                     else if($search == '4')
                     {
                      ...
                      if($search == 4 )
                      {
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.date as xdate,x.status as xstatus,y.fullname, z.*  FROM aset_req_details x,mstr_emp y,aset_req_to z WHERE z.req_by = y.id   AND z.status = '0' AND x.req_by = '$emp_id' AND x.id = z.req_id AND x.status = '1'  AND $qut GROUP BY x.asset_req_no ORDER BY x.id"); 
                      }
                      else if($search == 6 || $search == 8)
                      {
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.status as xstatus,x.date as xdate ,y.*,z.fullname,w.*  FROM aset_req_details x, aset_req_to y , mstr_emp z,aset_req_update w WHERE x.id = y.req_id AND $qut AND x.status = '1' AND x.req_by = z.id AND x.id = w.req_id AND x.req_by = '$emp_id' GROUP BY x.asset_req_no ORDER BY x.id"); 
                      }
                        else if($search == 7 )
                      {
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.status as xstatus,x.date as xdate ,y.*,z.fullname,w.*  FROM aset_req_details x, aset_req_to y , mstr_emp z,aset_req_update w WHERE x.id = y.req_id AND $qut AND x.status = '1' AND w.req_to = z.id AND x.id = w.req_id AND x.req_by = '$emp_id' GROUP BY x.asset_req_no ORDER BY x.id"); 
                      }
                      else{
                        $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.status as xstatus,x.date as xdate ,y.*,z.fullname  FROM aset_req_details x, aset_req_to y , mstr_emp z WHERE x.id = y.req_id AND $qut AND x.status = '1' AND x.req_by = z.id  AND x.req_by = '$emp_id' GROUP BY x.asset_req_no ORDER BY x.id"); 
                      }
              ...
              // echo $query_ftch->req_id;
                    ...
                    {
                $result = mysqli_query($con,"SELECT x.*,x.id as xid,x.status as xstatus,x.date as xdate ,y.*,z.fullname  FROM aset_req_details x, aset_req_to y , mstr_emp z WHERE x.id = y.req_id AND x.status = '1' AND x.req_by = z.id AND x.req_by = '$emp_id' GROUP BY x.asset_req_no ORDER BY x.id limit $start,$limit"); 
         ...
         while($fetch=mysqli_fetch_object($result))
        {
          ?>
                      ...
                      $abs = mysqli_query($con,"SELECT x.*,x.status as xstatus,y.fullname FROM aset_req_update x, mstr_emp y WHERE x.req_id = '$id_req' AND x.req_to = y.id ORDER BY x.id DESC LIMIT 1");
                      if(mysqli_num_rows($abs) > 0)
                      {
                        $fetch_update = mysqli_fetch_object( $abs);
                        ?>
                        <td><?php echo $fetch_update->req_date; ?></td>
                       <td><?php echo $fetch_update->fullname; ?></td>
                      <?php
                          if($fetch_update->xstatus == '1')
                          {
                            ?>
                            <td>approved</td>
                            <?php
                          }
                          else if($fetch_update->xstatus == '2')
                          {
                        ?>
                          <td>rejected</td>
                        <?php
                    ...
                    ?>
                        <td>
                            ...
                            <a href="aset_req_accept.php?id=<?php echo $fetch->xid; ?>&a_id=<?php echo $fetch->asset_id;?>" style="text-decoration:none;" class="btn btn-primary btn-xs" ><b>update</b></a>
                                <a href="asset_req_accept_view.php?id=<?php echo $fetch->xid; ?>&a_id=<?php echo $fetch->asset_id;?>" style="text-decoration:none;"></a>
