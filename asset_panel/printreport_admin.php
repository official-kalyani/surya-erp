               ...
               if(($_GET['sdate']  != "") && ($_GET['tdate']  != ""))
                ...
                if($search == 1)
                {
                    ...
                    $result = mysqli_query($con,"SELECT x.*,x.description as xdescription,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.asset_type = 'Admin' AND x.inv_no LIKE '%$spname%' ");
                     ...
                     {
                     $spname = $_GET['spname'];
                    $result = mysqli_query($con,"SELECT x.*,x.description as xdescription,y.sub_type FROM  aset_stock_entry x ,  aset_item_creation  y WHERE x.item_name = y.id AND x.asset_type = 'Admin' AND x.qty LIKE '%$spname%' ");
                }
                else if($search == 8)
                     {
                     $spname = $_GET['spname'];
                    ...
                    if(mysqli_num_rows($abc) > 0)
                    {
                      $result = mysqli_query($con,"SELECT x.*, x.id as xid,x.description as xdescription, y.*, y.id as id ,z.* FROM aset_stock_entry  x, vw_aset_stock_assaignment y, aset_item_creation z WHERE x.auto_serial = y.asset_no AND x.item_name = z.id AND x.asset_type = 'Admin' AND y.project_site LIKE '%$spname%' AND y.status = 1  GROUP BY x.auto_serial");
                          ...
                          $result = mysqli_query($con,"SELECT x.*, x.id as xid,x.description as xdescription, y.*, y.id as id ,z.* FROM aset_stock_entry  x, vw_aset_stock_assaignment y, aset_item_creation z WHERE x.auto_serial = y.asset_no AND x.item_name = z.id AND x.asset_type = 'Admin' AND x.godowo LIKE '%$spname%'  GROUP BY x.auto_serial");
                ...
                $result = mysqli_query($con,"SELECT x.*,x.description as xdescription,y.sub_type,z.* FROM  aset_stock_entry x ,  aset_item_creation  y , fin_organisation z WHERE x.item_name = y.id AND x.organisation = z.id AND x.organisation = '$spname' AND x.asset_type = 'Admin'  GROUP BY x.auto_serial");
              }
                 else if($search == 14)
              {
